...
4. ToastHub delegates rendering to **`DesktopToastService`** (WPF popup/stack renderer).

**Rule:** *No module should call `DesktopToastService` directly.* `DesktopToastService` is renderer-only; `ToastHub` owns policy.

---

## 2) Core files

### `MAIN/VAL.App/Host/DesktopToastService.cs`
...
- Call `DesktopToastService` to render the final toast.

### `MAIN/VAL.App/Host/ToastArchitectureMap.md`
...
### `MAIN/VAL.Continuum/Pipeline/00_Common/ToastLedgerService.cs`
...
**`MAIN/VAL.Continuum/Pipeline/00_Common/ContinuumHost.cs`**
...
ContinuumHost should call **ToastHub** keys (not DesktopToastService).

### B) Telemetry (truth size nudges)
**`MAIN/VAL.Continuum/Pipeline/06_Telemetry/Telemetry.ThresholdMonitor.cs`**
...
**`MAIN/VAL.App/MainWindow.xaml.cs`** (or Void host handler)
...
- **Toast text / duration / grouping / once-per-chat:** `ToastHubService` (and its toast definitions)
- **Toast key list:** `ToastKey.cs`
- **Toast visuals / positioning:** `DesktopToastService.cs`
- **Continuum triggers:** `ContinuumHost.cs`
- **Telemetry thresholds:** `Telemetry.ThresholdMonitor.cs`
- **Once-per-chat persistence:** `ToastLedgerService.cs`
