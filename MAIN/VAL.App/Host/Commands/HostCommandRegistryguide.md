...
- `VAL.Host/SessionContext.cs`
  - The single authoritative place for “what chatId are we in”.

Related (often used by handlers):

- `VAL.App/Host/ToastHubService.cs` + `VAL.Host/Toast/ToastKey.cs`
  - Centralized toast routing/policy (use this instead of calling `DesktopToastService` from random places).
...
`VAL.App/Host/Commands/<YourModule>CommandHandlers.cs`
...
- If a handler triggers a toast, call `ToastHub` (not `DesktopToastService`) so policy remains centralized.
...
`MAIN/VAL.App/Host/Commands/HostCommandRegistryguide.md`
