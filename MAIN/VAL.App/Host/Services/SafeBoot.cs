...
namespace VAL.App.Host.Services
{
    public sealed class SafeBoot
    {
        ...
        internal static int ResolveFatalExitCode(bool smokeTestEnabled)
        {
            return smokeTestEnabled ? 40 : 1;
        }
        ...
        {
            FallbackUsed = true;
            Environment.ExitCode = ResolveFatalExitCode(_smokeSettings.Enabled);
            ...
            _dialogService.ShowError($"Application startup failed: {exception.Message}", "Error");
#else
            if (_smokeSettings.Enabled)
            {
                return;
            ...
            try
            {
                _dialogService.ShowError("VAL failed to start. Please check the logs for details.", "VAL");
            }
            catch
            {
